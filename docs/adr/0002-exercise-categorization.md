...
Both exercise-browsing surfaces group by Primary Muscle Group first, with a name-only search box narrowing within those sections rather than replacing them, and no bottom-sheet filter panel. Muscle Group is the structural browse axis; anything else is additive narrowing on top of it. (This originally specified shadcn's `Command` component and ruled out filter chips and category tabs; the implementation used a plain search input over grouped lists instead, and a Type segmented control plus a jump-to-group rail were subsequently added — see `docs/adr/0003-shared-exercise-browser.md`, which supersedes this paragraph's account of the UI. Everything below about the data model still holds.)
